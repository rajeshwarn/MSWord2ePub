...
namespace Word2HTML4ePub
{
    ...
    internal partial class OPFFile
    {
        ...
        public static void UpdateRessources(string PackagePath, List<string> FilesToAddToManifest)
        {
            ...
            listOfFileToDelete.AddRange((from XmlNode xml in nodemanifest.ChildNodes where xml.Attributes["id"].Value.Contains("autoToc") select xml).ToList());
            ...
            for (int i = 0; i < FilesToAddToManifest.Count - 1; i++)
            {
                ...
                string contenu = File.ReadAllText(FinalFiles[i]);
                if (contenu.Contains("<svg"))
                    newElem.SetAttribute("properties", "svg"); 
                nodemanifest.AppendChild(newElem);
