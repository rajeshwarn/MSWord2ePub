...
namespace Word2HTML4ePub
{
    public partial class WordHTML2ePubHTML
    {
        ...
        private class NavTable
        {
            ...
            public void ExportNavTable(string FileName)
            {
            ...
            public void ExportNavTableSplittedbyChap(string FileNameBase)
            {
                ...
                for (int i = 0; i < loTitre.Count; i++)
                {
                    string nodestr = "<li><a href=\"" + Path.GetFileName(FileNameBase) + "-" + loTitre[i].NumeroDuTitre.ToString() + ".html#" + loTitre[i].GetID() + "\">" + loTitre[i].getTitre() + "</a></li>\r";
                    int newLevel = loTitre[i].getLevel();
                    if (lastlevel < newLevel)
                    {
                        navstr += "<ol>\r" + nodestr;
